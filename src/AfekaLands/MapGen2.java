...
public abstract class MapGen2 implements Runnable {
		...
		for(int i=playerPOS[0]-20;i<playerPOS[0]+20;i++)
			for(int j=playerPOS[1]-20;j<playerPOS[1]+20;j++)
				if(i>=0 && j>=0 && i<mapRAW[0].length && j<mapRAW[0][0].length) {
					if(i>=playerPOS[0]-4 && i<=playerPOS[0]+4 && j>=playerPOS[1]-4 && j<=playerPOS[1]+4) {
						if(i!=lastrow)
							sb.append("\n");
					...
					if(i>=playerPOS[0]-10 && i<=playerPOS[0]+10 && j>=playerPOS[1]-10 && j<=playerPOS[1]+10) {
						if(mapRAW[1][i][j]!=null && mapRAW[1][i][j].contains("E"))
							localEnemies++;
					}
					else
						if(mapRAW[1][i][j]!=null && mapRAW[1][i][j].contains("E") && !allEnemies.isEmpty())
							for(Enemy e : allEnemies)
	...
	public static String Convert(String rawMAP) {
		...
		finalMAP=finalMAP.replace("#", "🌳").replace("!", "🌾").replace("~", "🟦").replace("^", "⛰").replace("P", "🧝‍♂️").replace("$", "🏘").replace("E", "👹").replace("*","🌲");
		return finalMAP;
	}
			...
			for(int i=0;i<mapRAW[0].length;i++)
				for(int j=0;j<mapRAW[0][i].length;j++) {
					if(mapRAW[0][i][j].contains("$"))
						CoronaBot.shops.add(new Shop(i,j));
				}
	...
	public static void movePlayer(char direction) {
		if((direction == 'w') || (direction == 'a') || (direction == 's') || (direction == 'd')) {
			...
			try {
				...
				if(mapRAW[1][playerPOS[0]][playerPOS[1]]!=null && mapRAW[1][playerPOS[0]][playerPOS[1]].contains("E")) {
					isTackle=true;
					for(Enemy e : allEnemies)
