...
public class Shop {
	...
	public Shop(int i, int j) {
		...
		if (rand.nextInt(2) == 0)
			inv.add(new Item(Sort.HP, rand.nextInt(AfekaLandsController.player1.getLevel() * (rand.nextInt(10) + 1))));
		if (rand.nextInt(2) == 0)
			inv.add(new Item(Sort.EP, rand.nextInt(AfekaLandsController.player1.getLevel() * (rand.nextInt(3) + 1))));

		inv.add(new Item(Sort.BEER, rand.nextInt(10) + 2));

		if (rand.nextInt(5) == 0)
			inv.add(new Weapon(AfekaLandsController.player1.getLevel()));
		if (rand.nextInt(5) == 0)
			inv.add(new Weapon(AfekaLandsController.player1.getLevel()));
		if (rand.nextInt(4) == 0)
			inv.add(new Armor(kinds[rand.nextInt(kinds.length)], AfekaLandsController.player1.getLevel()));
		if (rand.nextInt(4) == 0)
			inv.add(new Armor(kinds[rand.nextInt(kinds.length)], AfekaLandsController.player1.getLevel()));
	}
