		...
		try {
			...
			for(int i=1;i<4;i++) {
				try {
					...
					if(level[i].contains(CoronaBot.player1.getName() + " [L]" + CoronaBot.player1.getLevel()))
						break;
	
					if((Integer.parseInt(level[i].substring(level[i].indexOf("[")+3))<CoronaBot.player1.getLevel())) {
						temp1 = level[i];
						level[i] = CoronaBot.player1.getName() + " [L]" + CoronaBot.player1.getLevel();
						for(int j=i;j<4;j++)
							if(j<3) {
								temp2=level[j+1];
								level[j+1] = temp1;
