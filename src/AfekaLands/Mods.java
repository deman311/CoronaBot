...
public interface Mods {
	enum wMOD {
		TRASH, WEAK, GOOD, STRONG, EPIC, GODLY
	};

	enum eMOD {
		BABY, WEAK, TOUGH, STRONG, BOSS
	};
	...
	public static Enemy modEnemy(Enemy e1, eMOD mod) {
		...
		case STRONG:
			...
			e1.addLoot(Sort.COIN, AfekaLandsController.player1.getLevel() * 2
					+ rand.nextInt(AfekaLandsController.player1.getLevel()));
			e1.addWeapon(1);
			e1.addArmor(1);
			break;
		case BOSS:
			e1.setName("[BOSS] " + e1.getName());
			if (e1.getLevel() < AfekaLandsController.player1.getLevel())
				e1 = new Enemy(AfekaLandsController.player1.getLevel());
			e1.setHP((int) (e1.getHP() * 3));
			e1.setMaxHP(e1.getHP());
			e1.setDEF((int) (e1.getDEF() * 1.5));
			e1.setDMG((int) (e1.getDamage() * 2));
			e1.setEXP((int) (e1.getEXP() * 5));
			e1.addLoot(Sort.COIN, AfekaLandsController.player1.getLevel() * 4
					+ rand.nextInt(AfekaLandsController.player1.getLevel()));
			e1.addLoot(Sort.HP, AfekaLandsController.player1.getLevel() * (1 + rand.nextInt(2)));
			e1.addLoot(Sort.EP, AfekaLandsController.player1.getLevel() * (1 + rand.nextInt(2)));
			e1.addWeapon(3);
			e1.addArmor(3);
			break;
		}
