...
public class Enemy {
	...
	public Enemy() {
		...
		if(rand.nextInt(3)>0)
			rando*=-1;
		if(CoronaBot.player1.getLevel()>5)
			level = CoronaBot.player1.getLevel()+rando;
		else
		...
		if(CoronaBot.player1.getLevel()<level)
			exp +=0.25*(level-CoronaBot.player1.getLevel());
		else if(CoronaBot.player1.getLevel()>level+1)
			exp -=0.25*(CoronaBot.player1.getLevel()-level);
	...
	public void addWeapon(int amount) {
		Weapon wep=null;
		int level=CoronaBot.player1.getLevel();
